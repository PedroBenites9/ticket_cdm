            ...
            <div className="modal-content border-0 shadow-2xl" style={{ borderRadius: '20px', overflow: 'hidden' }}>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body p-4 bg-light">
                  ...
                  {tarea.descripcion && (
                    <div className="bg-white p-3 rounded-4 shadow-sm mb-4 border border-warning border-opacity-25" style={{ backgroundColor: '#fffdf6' }}>
                      <p className="text-muted small mb-2 ls-1 fw-bold uppercase"><span className="me-1">📝</span> Instrucciones / Checklist</p>
                      <style dangerouslySetInnerHTML={{ __html: `
                        .instrucciones-content ol { padding-left: 20px; margin-bottom: 0; }
                        .instrucciones-content ul { padding-left: 20px; list-style-type: disc; margin-bottom: 0; }
                        .instrucciones-content ul li[data-list="bullet"] { list-style-type: disc; }
                        .instrucciones-content ul li[data-list="check"] { list-style-type: none; position: relative; }
                        .instrucciones-content ul li[data-list="check"]::before { content: "☐"; position: absolute; left: -20px; }
                      `}}></style>
                      <div className="font-monospace text-dark small instrucciones-content" dangerouslySetInnerHTML={{ __html: tarea.descripcion }}></div>
                    </div>
                  )}

                  {/* Planillas Base adjuntas desde la creación de la tarea */}
                  {tarea.planillas_adjuntas && (() => {
                      let planillasBase = [];
                      try {
                          planillasBase = JSON.parse(tarea.planillas_adjuntas);
                      } catch {
                          if (Array.isArray(tarea.planillas_adjuntas)) planillasBase = tarea.planillas_adjuntas;
                      }
                      if (planillasBase.length === 0) return null;
                      return (
                          <div className="bg-white p-3 rounded-4 shadow-sm mb-4 border border-primary border-opacity-25" style={{ backgroundColor: '#f5fbff' }}>
                            <p className="text-muted small mb-2 ls-1 fw-bold uppercase"><span className="me-1">📁</span> Planillas / Archivos de Referencia a utilizar</p>
                            <div className="d-flex flex-column gap-2 mt-2">
                                {planillasBase.map((arch, idx) => (
                                    <a key={`plantilla-${idx}`} 
                                       href={`${URL_API.replace('/api', '')}/api/tareas/archivo/${arch}`} 
                                       target="_blank" rel="noreferrer"
                                       className="btn btn-sm btn-outline-primary d-flex align-items-center justify-content-start gap-2 shadow-sm"
                                       style={{ borderRadius: '10px' }}
                                    >
                                       <span>⬇️</span> <span className="text-truncate flex-grow-1 text-start fw-bold">{arch}</span>
                                    </a>
                                ))}
                            </div>
                          </div>
