...
const INFO_PRIORIDAD = {
  Baja: {
    ...
    bordeColor: '#d1e7dd',
    tituloColor: '#0f5132',
    descripcion: 'Solicitudes de nuevos usuarios, mejoras estéticas o tareas de mantenimiento.',
    respuesta: '24 horas (1 a 2 días hábiles)',
    resolucion: '7 a 10 días hábiles'
  },
  Media: {
    titulo: 'Media',
    icono: '⚡',
    ...
    bordeColor: '#fef0cd',
    tituloColor: '#664d03',
    descripcion: 'Falla menor o pregunta técnica. El usuario opera con normalidad casi total.',
    respuesta: '8 horas (1 día hábil)',
    resolucion: '3 a 5 días hábiles'
  },
  Alta: {
    titulo: 'Alta',
    icono: '🔥',
    ...
    bordeColor: '#fbe2cd',
    tituloColor: '#a13e00',
    descripcion: 'Falla grave en un módulo. Afecta a un grupo, pero hay un proceso alternativo manual.',
    respuesta: '4 horas',
    resolucion: '24 a 48 horas (1 a 2 días hábiles)'
  },
  Urgente: {
    titulo: 'Urgente',
    icono: '🚨',
    ...
    bordeColor: '#f8d7da',
    tituloColor: '#842029',
    descripcion: 'Sistema principal caído. Operación detenida por completo. Sin alternativa temporal.',
    respuesta: '1 a 2 horas (En horario laboral)',
    resolucion: '8 horas (1 día laborable completo)'
  }
};
...
const ModalTicket = ({
  ...
  const manejarCierreModal = () => {
    // Verificamos si hay texto en la bitácora o archivos adjuntos sin enviar
    const textoLimpio = nuevoComentario ? nuevoComentario.replace(/<(.|\n)*?>/g, '').trim() : '';
    const hayArchivos = archivosBitacora && archivosBitacora.length > 0;
    
    // Verificamos si es un ticket nuevo que ya tiene datos
    const descripcionLimpia = formulario.descripcion ? formulario.descripcion.trim() : '';
    const creandoYConCambios = !editandoId && descripcionLimpia !== '';

    if (textoLimpio !== '' || hayArchivos) {
        if (!window.confirm("⚠️ No has enviado el mensaje en la bitácora.\n\n¿Deseas cerrar la ventana igualmente? (se perderá todo lo escrito)")) {
            return; // Abortamos el cierre
        }
    } else if (creandoYConCambios) {
        if (!window.confirm("⚠️ Estás creando un ticket nuevo sin guardar.\n\n¿Deseas cerrar la ventana igualmente? (se perderá todo lo escrito)")) {
            return; // Abortamos el cierre
        }
    }

    // Si confirma o si no había nada pendiente, cerramos normal
    ...
    <div className="modal d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1070 }}>
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div className="modal-content shadow-lg border-0" style={{ borderRadius: '15px' }}>
          <div className="modal-header bg-white border-bottom-0 pt-4 px-4 d-flex justify-content-between align-items-start">
            <div>
              <h5 className="modal-title fw-bold text-dark mb-1">
                {editandoId ? "🎫 Gestión de Ticket" : "📢 Nueva Incidencia Soporte IT"}
              </h5>
              {editandoId && formulario.fecha_creacion && (
                <small className="text-muted fw-medium d-block">
                  📅 Fecha de creación: {new Date(formulario.fecha_creacion).toLocaleString('es-AR', {
                      day: '2-digit', 
                      month: '2-digit', 
                      year: 'numeric', 
                      hour: '2-digit', 
            ...
            <button type="button" className="btn-close" onClick={manejarCierreModal}></button>
          </div>
          <div className="modal-body px-4">
            <form id="formTicket" onSubmit={guardarTicket}>
                ...
                <input type="text" className="form-control border-light shadow-sm bg-light bg-opacity-50 bg-body-secondary" name="asunto" value={formulario.asunto || ''} onChange={manejarCambio} required disabled={esSoloLectura} style={{ borderRadius: '10px' }} />
              </div>
                ...
                <div className={`col-md-${esGuardia ? '4' : '3'} mb-3`}>
                  ...
                  <select className="form-select border-light shadow-sm" name="prioridad" value={formulario.prioridad || ''} onChange={manejarCambio} disabled={esSoloLectura} style={{ borderRadius: '10px' }}>
                    ...
                    <option value="Urgente">🚨 Urgente</option>
                            ...
                            {/* SLA de Prioridad Info Card */}
              {formulario.prioridad && INFO_PRIORIDAD[formulario.prioridad] && (
                <div className="mb-3 animate__animated animate__fadeIn">
                  <div 
                    className="p-3 rounded border shadow-2xs" 
                    style={{ 
                      backgroundColor: INFO_PRIORIDAD[formulario.prioridad].colorFondo,
                      borderColor: INFO_PRIORIDAD[formulario.prioridad].bordeColor,
                      transition: 'all 0.3s ease-in-out'
                    }}
                  >
                    <div className="d-flex align-items-center mb-2">
                      <span className="fs-5 me-2">{INFO_PRIORIDAD[formulario.prioridad].icono}</span>
                      <h6 className="mb-0 fw-bold" style={{ color: INFO_PRIORIDAD[formulario.prioridad].tituloColor }}>
                        Prioridad {INFO_PRIORIDAD[formulario.prioridad].titulo}
                      </h6>
                    </div>
                    
                    <p className="mb-3 text-secondary" style={{ fontSize: '0.85rem', lineHeight: '1.4' }}>
                      {INFO_PRIORIDAD[formulario.prioridad].descripcion}
                    </p>
                    
                    <div className="row g-2">
                      <div className="col-6">
                        <div className="bg-white p-2 rounded border border-light-subtle shadow-sm" style={{ fontSize: '0.8rem' }}>
                          <span className="text-muted d-block fw-bold mb-1">⏱️ Respuesta Max.</span>
                          <span className="fw-semibold text-dark">{INFO_PRIORIDAD[formulario.prioridad].respuesta}</span>
                        </div>
                      </div>
                      <div className="col-6">
                        <div className="bg-white p-2 rounded border border-light-subtle shadow-sm" style={{ fontSize: '0.8rem' }}>
                          <span className="text-muted d-block fw-bold mb-1">✅ Resolución Max.</span>
                          <span className="fw-semibold text-dark">{INFO_PRIORIDAD[formulario.prioridad].resolucion}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mb-3">
                ...
                <textarea className="form-control border-light shadow-sm bg-body-secondary" rows="3" name="descripcion" value={formulario.descripcion || ''} onChange={manejarCambio} onPaste={manejarPegadoNuevoTicket} required disabled={editandoId && (usuarioLogueado !== formulario.solicitante)} style={{ borderRadius: '10px' }}></textarea>
              </div>
          ...
          <div className="modal-footer bg-light border-0 pb-4 px-4">
            <button type="button" className="btn btn-link text-muted text-decoration-none fw-bold me-auto" onClick={manejarCierreModal}>Cerrar</button>
